...
async function readArtifact(scope) {
	...
	const maxChars = Math.min(4e3, Math.max(1, Math.floor(scope.maxChars ?? 4e3)));
	return {
		artifactId,
...
function indexFetchResult(urlIndex, url, text, artifactId) {
	const normalized = normalizeUrl(url);
	if (normalized === "") return;
	urlIndex.set(normalized, {
		url: normalized,
		text: clip(text, MAX_URL_TEXT_FOR_VERIFY),
		artifactId
	});
}
...
let DeepResearchService = (() => {
	...
	return class DeepResearchService extends _classSuper {
		...
		async runInvestigation(id, run) {
			if (this.ctx.web === void 0) throw new Error("ctx.web is not available; mount a search provider and @deepseek-ai/dsh-web-fetch-http for research fetch");
		...
		async runCriterion(id, run, questionId, criterionId, dependencyContext) {
			...
			await this.spawnRole(id, run, {
				...
				register: (ctx) => {
					...
					ctx.tools.register(defineTool({
						...
						execute: async (args) => {
							...
							const artifact = body.trim() === "" ? null : await persistArtifact({
								projectId: id,
								questionId,
								criterionId,
								url,
					...
					ctx.tools.register(defineTool({
						...
						execute: async (args) => {
							...
							try {
								...
								toolsUsed += 1;
								await patchScout({
									activity: `Read artifact ${result.artifactId}`,
									scoutDraft: result.text.slice(0, 400)
								});
				...
				onDraft: (text) => {
					patchScout({ scoutDraft: text.slice(0, 800) });
				},
				lastText: (text) => {
					if (submitted) return;
							...
							try {
								...
								await this.update(id, (current) => {
									...
									return {
										...
										progress: mergeScoutProgress(current.progress, {
											...previous,
											evaluatorDraft: result.text.slice(0, 400),
				...
				onDraft: (text) => {
					this.update(id, (current) => {
						...
						return {
							...
							progress: mergeScoutProgress(current.progress, {
								...previous,
								evaluatorDraft: text.slice(0, 800)
							})
						};
					});
				},
...
function hydrateEvidence(item) {
	const sources = item.sources?.length ? item.sources : item.url || item.snippet ? [{
		url: item.url ?? "",
		snippet: item.snippet
	}] : [];
	return {
