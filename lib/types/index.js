...
let DeepResearchService = (() => {
    ...
    return class DeepResearchService extends _classSuper {
         ...
         * @returns the updated detached project.
         */
        confirmPlan(request) { return this.update(request.id, project => ({ ...project, planConfirmed: true, phase: 'investigating' })); }
        /**
        ...
        updateQuestion(request) {
            return this.update(request.id, project => {
                ...
                questions[index] = { ...current, status: request.status, criteria: request.criteria ?? current.criteria };
        ...
        complete(request) {
            return this.update(request.id, project => {
                ...
                if (project.evidence.length === 0)
                    throw new Error(`deepresearch: project ${project.id} has no evidence`);
         ...
         * @returns the updated detached project.
         */
        fail(request) { return this.update(request.id, project => ({ ...project, phase: request.aborted === true ? 'aborted' : 'failed', limitations: normalizeList([...project.limitations, requiredText(request.reason, 'reason')]) })); }
        /**
         ...
         */
        delete(request) { return this.enqueue(async () => { const table = this.requireTable(); const deleted = table.get(request.id) !== undefined; if (deleted)
        ...
        buildQuestions(input) {
            ...
            if (input.length > this.config.maxQuestions)
                throw new RangeError(`deepresearch: question limit ${this.config.maxQuestions} reached`);
            const ids = input.map(() => ResearchQuestionId(`rq-${randomUUID()}`));
            return input.map((item, index) => { if (item.criteria.length === 0)
                throw new TypeError(`deepresearch: questions[${index}] requires criteria`); if (item.criteria.length > this.config.maxCriteriaPerQuestion)
                throw new RangeError(`deepresearch: criterion limit ${this.config.maxCriteriaPerQuestion} reached`); return { id: ids[index], text: requiredText(item.text, `questions[${index}].text`), dependsOn: (item.dependsOn ?? []).map(dep => { const id = ids[dep]; if (id === undefined || dep >= index)
                    throw new TypeError(`deepresearch: questions[${index}] has invalid dependency ${dep}`); return id; }), status: 'pending', criteria: item.criteria.map((text, criterionIndex) => ({ id: `c${index + 1}.${criterionIndex + 1}`, text: requiredText(text, `questions[${index}].criteria[${criterionIndex}]`), status: 'missing', summary: '', gap: '' })) }; });
        }
        update(id, mutate) { return this.enqueue(async () => { const table = this.requireTable(); const current = table.get(id); if (current === undefined)
            throw new Error(`deepresearch: project ${id} not found`); const project = snapshot({ ...mutate(snapshot(current)), updatedAt: Math.max(Date.now(), current.updatedAt + 1) }); await table.put(id, project); return snapshot(project); }); }
...
function budgetFor(depth) { const limits = depth === 'quick' ? [8, 6] : depth === 'deep' ? [30, 24] : [18, 14]; return { maxSearches: limits[0], maxFetches: limits[1], searchesUsed: 0, fetchesUsed: 0 }; }
