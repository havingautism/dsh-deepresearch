...
let DeepResearchService = (() => {
    ...
    return class DeepResearchService extends _classSuper {
         ...
         */
        delete(request) { this.stopRun(request.id); return this.enqueue(async () => { const table = this.requireTable(); const deleted = table.get(request.id) !== undefined; if (deleted)
        ...
        launch(id, phase) {
            ...
            const run = { phase, controller: new AbortController() };
        ...
        async runProject(id, phase, run) {
            ...
            if (phase === 'investigating' && !allowed.includes('web_search'))
                throw new Error('web_search is not available in this profile');
            ...
            if (project === null)
                return;
            if (phase === 'planning' && project.phase === 'planning')
                throw new Error('the planning agent stopped before submitting a plan');
            if (phase === 'investigating' && ['investigating', 'ready_for_report', 'writing'].includes(project.phase))
