...
export { ResearchId } from "./types.js";
export { deepResearchDomainSpec, researchEvidenceSchema, researchProjectSchema } from "./spec.js";
    ...
    return class DeepResearchService extends _classSuper {
        ...
        start(request) {
            return this.enqueue(async () => {
                ...
                if (plan.length === 0)
                    throw new TypeError('deepresearch: plan must contain at least one step');
                const now = Date.now();
        ...
        addEvidence(request) {
            return this.enqueue(async () => {
                ...
                if (current.phase === 'complete')
                    throw new Error(`deepresearch: project ${request.id} is complete`);
        ...
        requireTable() {
            if (this.table === undefined)
                throw new Error('deepresearch: durable domain is not initialized');
            return this.table;
        }
    };
})();
...
function requiredText(value, field) {
    ...
    if (text === '')
        throw new TypeError(`deepresearch: ${field} must not be blank`);
    return text;
}
