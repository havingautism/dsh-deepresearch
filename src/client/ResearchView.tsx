...
function ResearchWorkspace({ project, api, t, onChange, onBack, onDelete, error, setError }: { project: ResearchProject; api: ResearchViewApi; t: ResearchViewProps['t']; onChange: (project: ResearchProject) => void; onBack: () => void; onDelete: () => Promise<void>; error: string | null; setError: (value: string | null) => void }) {
  ...
  useEffect(() => { setFocus(stepFor(project)); setGoal(project.goal); setConstraints(project.constraints); setQuestions(editablePlan(project)) }, [project])
  useEffect(() => {
...
function reachableStep(project: ResearchProject, step: ResearchStep): boolean { const order: ResearchStep[] = ['plan', 'investigate', 'report']; const unlocked = project.phase === 'ready_for_report' ? 'investigate' : stepFor(project); return order.indexOf(step) <= order.indexOf(unlocked) }
