...
export function ResearchView({ t, projectId, onSelectProject, onClose, ...api }: ResearchViewProps) {
  ...
  useEffect(() => {
    ...
    if (projectId === null) { setSelected(null); return }
    const listed = projects.find(item => item.id === projectId)
    if (listed !== undefined) { setSelected(listed); return }
    let active = true
    void api.get(projectId).then(project => { if (active) setSelected(project) }, cause => { if (active) setError(messageOf(cause)) })
    return () => { active = false }
  }, [api, projectId, projects])
  ...
  if (selected !== null) return <ResearchWorkspace project={selected} api={api} t={t} onChange={updateSelected} onBack={() => { openProject(null) }} onDelete={async () => { await api.delete(selected.id); openProject(null); await refresh(query) }} error={error} setError={setError} />
...
function ResearchWorkspace({ project, api, t, onChange, onBack, onDelete, error, setError }: { project: ResearchProject; api: ResearchViewApi; t: ResearchViewProps['t']; onChange: (project: ResearchProject) => void; onBack: () => void; onDelete: () => Promise<void>; error: string | null; setError: (value: string | null) => void }) {
  const [focus, setFocus] = useState<ResearchStep>(stepFor(project)); const [goal, setGoal] = useState(project.goal); const [constraints, setConstraints] = useState(project.constraints); const [questions, setQuestions] = useState<ResearchStartRequest['questions']>(() => editablePlan(project)); const [busy, setBusy] = useState(false)
  useEffect(() => {
  ...
  return <div className={css.workspace}><header className={css.workspaceHeader}><button className={css.backButton} type="button" onClick={onBack}><IconArrowLeft size={15} />{t('workspace.back')}</button><div className={css.projectHeading}><p className={css.eyebrow}>RESEARCH PROJECT</p><h2>{project.title}</h2></div><div className={css.headerActions}><span className={css.phase} data-phase={project.phase}>{phaseLabel(project.phase, t)}</span><button className={css.deleteText} type="button" onClick={() => { void onDelete() }}>{t('workspace.delete')}</button></div></header><div className={css.progressBar}><nav className={css.stepper}>{([['plan', 'stepper.plan'], ['investigate', 'stepper.investigate'], ['report', 'stepper.report']] as const).map(([id, key]) => { const reachable = reachableStep(project, id); return <button key={id} type="button" disabled={!reachable} data-active={activeStep === id} onClick={() => { setFocus(id) }}>{t(key)}</button> })}</nav></div>{error === null ? null : <div className={css.error}>{error}</div>}<div className={css.detailBody}>
    ...
    {activeStep === 'report' ? <section className={css.reportPane}><div className={css.sectionHeader}><div><h3>{t('report.title')}</h3><p>{t('report.subtitle')}</p></div></div>{project.report ? <article className={css.reportDocument}><MarkdownText text={project.report} streaming={project.phase === 'writing'} /></article> : project.phase === 'writing' ? <div className={css.reportPending} role="status"><IconLoading className={css.spinner} size={16} /><span>{t('report.writing')}</span></div> : <div className={css.reportPending}>{t('report.empty')}</div>}{project.limitations.length > 0 ? <section className={css.reportLimitations}><h4>{t('report.limitations')}</h4><ul>{project.limitations.map(item => <li key={item}>{item}</li>)}</ul></section> : null}</section> : null}
  </div></div>
}
...
function reachableStep(project: ResearchProject, step: ResearchStep): boolean {
  if (project.planConfirmed && ['done', 'incomplete', 'failed', 'aborted'].includes(project.phase)) return true
