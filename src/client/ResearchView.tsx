...
export function ResearchView({ t, ...api }: ResearchViewProps) {
  ...
  const refresh = useCallback(async (nextQuery: string) => { setError(null); try { const next = await api.list(nextQuery); setProjects(next); setSelected(current => { if (current === null) return null; const listed = next.find(item => item.id === current.id); return listed !== undefined && listed.updatedAt >= current.updatedAt ? listed : current }) } catch (cause) { setError(messageOf(cause)) } }, [api])
  useEffect(() => {
  ...
  return <div className={css.shell} data-conversation-composer-overlay=""><div className={css.content}>
    <div className={css.toolbar}><div className={css.filters} aria-label={t('library.filterAria')}>{filters.map(([id, key]) => <button key={id} className={filter === id ? css.activeChip : css.chip} type="button" aria-current={filter === id ? 'page' : undefined} onClick={() => { setFilter(id) }}>{t(key)}</button>)}</div><div className={css.toolbarActions}><input className={css.search} value={query} onChange={event => { setQuery(event.target.value) }} placeholder={t('toolbar.search')} aria-label={t('toolbar.searchAria')} /><select className={css.select} value={sort} onChange={event => { setSort(event.target.value as 'recent' | 'title') }} aria-label={t('toolbar.sortAria')}><option value="recent">{t('toolbar.sortRecent')}</option><option value="title">{t('toolbar.sortTitle')}</option></select><div className={css.viewToggle}><button className={css.iconButton} type="button" aria-label={t('toolbar.gridView')} aria-pressed={viewMode === 'grid'} data-active={viewMode === 'grid'} onClick={() => { setViewMode('grid') }}>▦</button><button className={css.iconButton} type="button" aria-label={t('toolbar.listView')} aria-pressed={viewMode === 'list'} data-active={viewMode === 'list'} onClick={() => { setViewMode('list') }}>☷</button></div><button className={css.primaryButton} type="button" onClick={() => { setError(null); setComposerOpen(true) }}>{t('action.start')}</button></div></div>
    <section className={css.library}><header className={css.libraryTitle}><div><h2>{t('library.title')}</h2><p>{t('library.projectCount', { count: visible.length })}</p></div></header>
      ...
      {visible.length === 0 ? <div className={css.emptyState}><span aria-hidden="true">⚡</span><strong>{query === '' ? t('empty.none') : t('empty.noMatch')}</strong><p>{query === '' ? t('empty.hintStart') : t('empty.hintNoMatch')}</p>{query === '' ? <button className={css.primaryButton} type="button" onClick={() => { setComposerOpen(true) }}>{t('action.start')}</button> : null}</div> : <div className={viewMode === 'grid' ? css.projectGrid : css.projectList}>{viewMode === 'grid' ? <button className={css.createCard} type="button" onClick={() => { setComposerOpen(true) }}><span>＋</span><strong>{t('action.startShort')}</strong></button> : null}{visible.map((project, index) => <ProjectCard key={project.id} project={project} index={index} list={viewMode === 'list'} t={t} onOpen={() => { setSelected(project) }} onDelete={() => { void api.delete(project.id).then(() => refresh(query), cause => { setError(messageOf(cause)) }) }} />)}</div>}
    </section>
...
function ProjectCard({ project, index, list, t, onOpen, onDelete }: { project: ResearchProject; index: number; list: boolean; t: ResearchViewProps['t']; onOpen: () => void; onDelete: () => void }) { const [emoji, title] = splitEmoji(project.title); return <article className={css.projectCard} data-list={list || undefined} style={{ '--card-tint': CARD_TONES[index % CARD_TONES.length] } as React.CSSProperties}><button className={css.cardOpen} type="button" onClick={onOpen} aria-label={t('card.openAria', { title: project.title })} /><div className={css.cardEmoji}>{emoji || '⚡'}</div><div className={css.cardInfo}><h3>{title}</h3><p>{project.goal || project.question}</p><div><span className={css.phase} data-phase={project.phase}>{phaseLabel(project.phase, t)}</span><span>{t('card.evidence', { count: project.evidence.length, date: formatDate(project.updatedAt) })}</span></div></div><button className={css.deleteButton} type="button" onClick={event => { event.stopPropagation(); onDelete() }}>×</button></article> }
...
function ResearchWorkspace({ project, api, t, onChange, onBack, onDelete, error, setError }: { project: ResearchProject; api: ResearchViewApi & Pick<ConvViewProps, 'inputActions'>; t: ResearchViewProps['t']; onChange: (project: ResearchProject) => void; onBack: () => void; onDelete: () => Promise<void>; error: string | null; setError: (value: string | null) => void }) {
  ...
  useEffect(() => {
    if (!['investigating', 'ready_for_report', 'writing'].includes(project.phase)) return
    let active = true
    const refresh = async () => {
      try {
  ...
  return <div className={css.workspace} data-conversation-composer-overlay=""><header className={css.workspaceHeader}><button className={css.backButton} type="button" onClick={onBack}>{t('workspace.back')}</button><div className={css.projectHeading}><p className={css.eyebrow}>RESEARCH PROJECT</p><h2>{project.title}</h2></div><div className={css.headerActions}><span className={css.phase} data-phase={project.phase}>{phaseLabel(project.phase, t)}</span><button className={css.deleteText} type="button" onClick={() => { void onDelete() }}>{t('workspace.delete')}</button></div></header><div className={css.progressBar}><nav className={css.stepper}>{([['plan', 'stepper.plan'], ['investigate', 'stepper.investigate'], ['report', 'stepper.report']] as const).map(([id, key]) => { const reachable = reachableStep(project.phase, id); return <button key={id} type="button" disabled={!reachable} data-active={activeStep === id} onClick={() => { setFocus(id) }}>{t(key)}</button> })}</nav></div>{error === null ? null : <div className={css.error}>{error}</div>}<div className={css.detailBody}>
    {activeStep === 'plan' ? <section className={css.planPane}><div className={css.sectionHeader}><div><h3>{t('plan.title')}</h3><p>{t('plan.subtitle')}</p></div><div className={css.headerActions}>{project.planConfirmed ? <span className={css.confirmed}>{t('plan.confirmed')}</span> : <><button className={css.secondaryButton} type="button" disabled={busy} onClick={savePlan}>{t('action.saveChanges')}</button><button className={css.primaryButton} type="button" disabled={busy} onClick={() => { void confirmAndStart() }}>{t('action.confirmStart')}</button></>}</div></div><div className={css.formGrid}><label>{t('plan.goal')}<textarea value={goal} onChange={event => { setGoal(event.target.value) }} disabled={project.planConfirmed || busy} /></label><label>{t('plan.constraints')}<textarea value={constraints} onChange={event => { setConstraints(event.target.value) }} disabled={project.planConfirmed || busy} /></label></div><label className={css.fieldLabel}>{t('plan.criteria')}</label><textarea className={css.planEditorLarge} value={plan} onChange={event => { setPlan(event.target.value) }} disabled={project.planConfirmed || busy} /></section> : null}
    {activeStep === 'investigate' ? <section className={css.investigatePane}><header className={css.questionHeader}><span>{t('composer.question')}</span><h3>{project.question}</h3>{project.goal ? <p>{project.goal}</p> : null}</header><div className={css.metrics}><Metric label={t('metric.subQuestions')} value={`${project.questions.filter(q => ['covered', 'partial', 'blocked'].includes(q.status)).length}/${project.questions.length}`} /><Metric label={t('metric.evidence')} value={String(project.evidence.length)} /><Metric label={t('metric.searchBudget')} value={`${project.budget.searchesUsed}/${project.budget.maxSearches}`} /></div><div className={css.sectionHeader}><div><h3>{t('investigate.title')}</h3><p>{t('investigate.subtitle')}</p></div>{project.phase === 'investigating' ? <button className={css.stopButton} type="button" disabled={busy} onClick={() => { void action(() => api.fail(project.id, t('investigate.stopReason'), true)) }}>{t('investigate.stop')}</button> : null}</div><div className={css.boardGrid}><div className={css.questions}>{project.questions.map((question, index) => <article className={css.questionCard} key={question.id}><div className={css.questionTitle}><span>{String(index + 1).padStart(2, '0')}</span><div><h4>{question.text}</h4><strong data-status={question.status}>{statusLabel(question.status, t)}</strong></div></div>{question.dependsOn.length > 0 ? <p>{t('investigate.dependsOn', { count: question.dependsOn.length })}</p> : null}<ul>{question.criteria.map(criterion => <li key={criterion.id} data-status={criterion.status}><span>{criterion.status === 'covered' ? '✓' : '○'}</span><div><b>{criterion.text}</b>{criterion.summary ? <p>{criterion.summary}</p> : null}{criterion.gap ? <em>{criterion.gap}</em> : null}</div></li>)}</ul></article>)}</div><aside className={css.evidencePane}><h3>{t('evidence.title')} <span>{project.evidence.length}</span></h3>{project.evidence.length === 0 ? <div className={css.evidenceEmpty}><span>◎</span><p>{t('evidence.empty')}</p></div> : project.evidence.map(item => <article className={css.evidenceCard} key={item.id}><div><strong>{item.source}</strong><span data-confidence={item.confidence}>{item.confidence}</span></div><p>{item.claim}</p>{item.snippet ? <blockquote>{item.snippet}</blockquote> : null}{item.url === null ? null : <a href={item.url} target="_blank" rel="noreferrer">{t('evidence.open')}</a>}</article>)}</aside></div></section> : null}
    {activeStep === 'report' ? <section className={css.reportPane}><div className={css.sectionHeader}><div><h3>{t('report.title')}</h3><p>{t('report.subtitle')}</p></div><div className={css.headerActions}><button className={css.secondaryButton} type="button" disabled={busy} onClick={() => { complete(true) }}>{t('action.savePartial')}</button><button className={css.primaryButton} type="button" disabled={busy} onClick={() => { complete(false) }}>{t('action.complete')}</button></div></div><textarea className={css.reportEditor} value={report} onChange={event => { setReport(event.target.value) }} placeholder={t('report.placeholder')} /><label className={css.fieldLabel}>{t('report.limitations')}</label><textarea className={css.limitationsEditor} value={limitations} onChange={event => { setLimitations(event.target.value) }} placeholder={t('report.limitationsPlaceholder')} /></section> : null}
  </div></div>
}

function Metric({ label, value }: { label: string; value: string }) { return <div><span>{label}</span><strong>{value}</strong></div> }
const CARD_TONES = ['#5b8def', '#3d9b8f', '#c27a4a', '#8b6bc9', '#4a9b6e', '#b85c7a']
function initialPlan(question: string, t: ResearchViewProps['t']): ResearchStartRequest['questions'] { return [
...
function investigationPrompt(project: ResearchProject): string {
